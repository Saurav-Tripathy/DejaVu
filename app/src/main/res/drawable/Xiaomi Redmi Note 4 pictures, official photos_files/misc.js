...
$gsm.onload(function () {
    ...
    if (becomeAFan) { // do nothing on non-specs pages        
        var phoenId = window.location.toString().match(/-(\d+)\.php/)[1];
        becomeAFan.onclick = function () {
            ...
            if (!loggedIn) {
                ...
                }, {idPhone: phoenId, nFan: fan? 1 : 0}, function () {
...
/* $gsm.onload(function () {
    $gsm.tag("script", {src: "http://cdn2.gsmarena.com/vv/bigpic/gradient-fallback.js?v=1"}, "", document.body);
 })*/
